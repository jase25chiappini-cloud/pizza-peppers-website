...
// ADD THIS ENTIRE BLOCK AFTER YOUR LAST IMPORT STATEMENT
...
// IMPORTANT: Replace this placeholder with the actual firebaseConfig object
...
// ADD THIS AUTHENTICATION CONTEXT BLOCK
const AuthContext = createContext();
function useAuth() { return useContext(AuthContext); }
// Find your AuthProvider function
function AuthProvider({ children }) {
    const [currentUser, setCurrentUser] = useState(null);
    const [loading, setLoading] = useState(true);
    const loginWithGoogle = () => { const provider = new GoogleAuthProvider(); return signInWithPopup(auth, provider); };
    ...
    useEffect(() => { const unsubscribe = onAuthStateChanged(auth, user => { setCurrentUser(user); setLoading(false); }); return unsubscribe; }, []);
    
    // --- UPDATE THE VALUE OBJECT ---
    const value = { currentUser, loginWithGoogle, loginWithApple, logout }; 
    // -------------------------------

    return <AuthContext.Provider value={value}>{!loading && children}</AuthContext.Provider>;
}

// ADD THIS LOGIN MODAL BLOCK
const GoogleIcon = () => ( <svg viewBox="0 0 24 24" width="24" height="24" style={{ marginRight: '1rem' }}><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg> );
// After your GoogleIcon component
const AppleIcon = () => ( <svg viewBox="0 0 24 24" width="24" height="24" style={{ marginRight: '1rem' }}><path d="M19.05 17.55C18.8 18.09 18.45 18.75 18 19.35C17.5 19.95 17.05 20.55 16.5 21C16 21.5 15.45 21.8 14.85 22.05C14.25 22.3 13.65 22.5 13 22.5C12.3 22.5 11.7 22.3 11.1 22.05C10.5 21.8 10 21.5 9.45 21C8.95 20.5 8.45 19.95 7.95 19.35C7.5 18.75 7.15 18.1 6.9 17.55C6.3 16.5 6 15.35 6 14.1C6 12.8 6.3 11.65 6.9 10.65C7.2 10.05 7.6 9.45 8.1 8.85C8.6 8.25 9.15 7.7 9.75 7.2C10.35 6.7 11 6.4 11.65 6.15C12.3 5.9 13 5.8 13.75 5.8C14.45 5.8 15.15 6 15.8 6.3C15.15 6.75 14.65 7.35 14.3 8.1C14 8.85 13.85 9.6 13.85 10.35C13.85 11.25 14.05 12.1 14.45 12.9C14.85 13.7 15.4 14.35 16.1 14.85C16.8 15.35 17.6 15.6 18.5 15.6C18.8 15.6 19.05 15.55 19.25 15.5C19.5 15.45 19.7 15.4 19.9 15.35C20.9 14.85 21.7 14.15 22.35 13.25C21.9 12.95 21.45 12.7 21 12.5C20.5 12.3 20 12.15 19.45 12.05C18.75 11.85 18.05 11.75 17.3 11.75C16.45 11.75 15.65 11.9 14.9 12.2C14.15 12.5 13.5 12.95 12.95 13.55C12.4 14.15 11.95 14.8 11.6 15.5C11.25 16.2 11.05 16.95 11.05 17.75C11.05 18.05 11.1 18.3 11.2 18.5C11.3 18.7 11.45 18.85 11.65 18.95C12.2 19.25 12.8 19.4 13.45 19.4C14.1 19.4 14.7 19.2 15.25 18.8C15.8 18.4 16.25 17.9 16.6 17.3C16.95 16.7 17.2 16.05 17.35 15.35C16.45 15.1 15.7 14.65 15.1 13.95C14.5 13.25 14.2 12.45 14.2 11.55C14.2 10.7 14.4 9.9 14.8 9.15C15.2 8.4 15.8 7.8 16.6 7.35C16.9 7.15 17.2 7.05 17.5 7.05C17.8 7.05 18.05 7.1 18.25 7.2C18.5 7.3 18.7 7.45 18.85 7.65C17.95 8.1 17.2 8.7 16.6 9.45C16 10.2 15.7 11.05 15.7 12C15.7 12.8 15.9 13.5 16.3 14.1C16.7 14.7 17.2 15.15 17.85 15.45C18.1 15.55 18.3 15.6 18.45 15.6C18.5 15.6 18.5 15.6 18.5 15.6C18.5 15.6 18.5 15.6 18.5 15.6C19.4 15.6 20.2 15.35 20.9 14.85C20.65 16.1 20 17 19.05 17.55ZM17.2 4.6C16.8 4.1 16.3 3.65 15.7 3.25C15.1 2.85 14.45 2.65 13.75 2.65C13.05 2.65 12.4 2.8 11.8 3.1C11.2 3.4 10.65 3.8 10.15 4.3C9.65 4.8 9.2 5.3 8.8 5.85C8.4 6.4 8.15 7 8.05 7.6C8.3 7.6 8.5 7.55 8.7 7.5C8.9 7.45 9.1 7.4 9.3 7.4C10.05 7.15 10.8 7 11.55 7C12.3 7 13.05 7.15 13.8 7.45C14.55 7.75 15.2 8.2 15.75 8.8C16.05 8.45 16.3 8.1 16.5 7.75C16.7 7.4 16.85 7.05 16.95 6.7C17.05 6.35 17.1 6 17.1 5.65C17.15 5.3 17.2 4.95 17.2 4.6Z" fill="currentColor"/></svg> );

const PhoneIcon = () => ( <svg viewBox="0 0 24 24" width="24" height="24" style={{ marginRight: '1rem' }} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.63A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg> );
// REPLACE your old LoginModal with this new version
function LoginModal({ onClose }) {
    const { loginWithGoogle, loginWithApple } = useAuth(); // <-- Add loginWithApple
    const handleGoogleLogin = async () => { try { await loginWithGoogle(); onClose(); } catch (error) { console.error("Failed to log in with Google", error); } };
    
    // --- ADD APPLE HANDLER ---
    const handleAppleLogin = async () => { try { await loginWithApple(); onClose(); } catch (error) { console.error("Failed to log in with Apple", error); } };

    // --- ADD A STATE FOR PHONE LOGIN UI ---
    const [showPhoneInput, setShowPhoneInput] = useState(false);
    // ------------------------------------

    const buttonStyle = { display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', padding: '0.75rem', marginBottom: '1rem', borderRadius: '0.5rem', border: '1px solid var(--border-color)', backgroundColor: 'var(--background-dark)', color: 'var(--text-light)', fontSize: '1rem', fontFamily: 'var(--font-heading)', cursor: 'pointer', transition: 'background-color 0.2s' };

    // --- PHONE UI ---
    // This is the screen that will show when you click "Continue with Phone"
    if (showPhoneInput) {
        return ( <div className="modal-overlay" onClick={onClose}><div className="modal-content" onClick={(e) => e.stopPropagation()} style={{maxWidth: '400px'}}>
            <div className="modal-header" style={{display: 'flex', alignItems: 'center'}}>
                <button onClick={() => setShowPhoneInput(false)} className="quantity-btn" style={{width: '2.5rem', height: '2.5rem', flexShrink: 0}}>&larr;</button>
                <h3 className="panel-title" style={{marginLeft: '1rem', marginRight: 'auto', marginBottom: 0}}>Enter Phone Number</h3>
                <button onClick={onClose} className="quantity-btn" style={{width: '2.5rem', height: '2.5rem', flexShrink: 0}}>&times;</button>
            </div>
            <div className="modal-body">
                <p style={{color: 'var(--text-medium)', fontSize: '0.9rem', textAlign: 'center', marginTop: 0}}>Phone login is not implemented yet.</p>
                {/* This is where you will need to add:
                  1. A phone number input field.
                  2. An element for the reCAPTCHA verifier (often a div).
                  3. A "Send Code" button.
                  4. Logic to handle the code input screen after the SMS is sent.
                */}
            </div>
        </div></div> );
    }
    ...
    // --- DEFAULT LOGIN UI ---
    // This is the main login modal screen
    return ( <div className="modal-overlay" onClick={onClose}><div className="modal-content" onClick={(e) => e.stopPropagation()} style={{maxWidth: '400px'}}>
        ...
        <div className="modal-body">
            <button style={buttonStyle} onClick={handleGoogleLogin}><GoogleIcon />Continue with Google</button>
            
            {/* --- ADD APPLE BUTTON --- */}
            <button style={buttonStyle} onClick={handleAppleLogin}><AppleIcon />Continue with Apple</button>
            
            {/* --- ADD PHONE BUTTON --- */}
            <button style={buttonStyle} onClick={() => setShowPhoneInput(true)}>
                <PhoneIcon />Continue with Phone
            </button>
            {/* ------------------------------- */}

            <p style={{textAlign: 'center', color: 'var(--text-medium)', fontSize: '0.8rem'}}>By continuing, you agree to our Terms and Conditions.</p>
        </div>
    </div></div> );
}
const deliveryZones = {
...
function AppStyles() {
  const styles = `
   ...
   /* --- CORRECTED FUTURISTIC FOOTER STYLES --- */
    @keyframes scan-glow {
      0% {
        left: -10%; /* Start just off-screen to the left */
      }
      100% {
        left: 110%; /* End just off-screen to the right */
      }
    }
    .site-footer {
      padding: 3rem 1rem;
        ...
        <div className="sub-panel-view">
            ...
            <div className="sub-panel-content">
                {Object.entries(deliveryZones).map(([suburb, price]) => (
                    <div key={suburb} style={{display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0', textTransform: 'capitalize', borderBottom: '1px solid var(--border-color)'}}>
...
function AboutPanel({ isMapsLoaded }) {
    ...
    useEffect(() => {
        if (isMapsLoaded && mapRef.current && currentView === 'main') {
            const map = new window.google.maps.Map(mapRef.current, {
                center: storeLocation, zoom: 15, disableDefaultUI: true,
    ...
    // Based on the currentView state, show the correct component
    ...
    return (
        <>
            ...
            <div className="about-panel-list-item"><a href="tel:0883877700"><h4>Call Us</h4><p>(08) 8387 7700</p></a></div>
            <div className="about-panel-list-item"><button onClick={() => setCurrentView('hours')}><h4>Opening Hours</h4><p>View our weekly trading hours</p></button></div>
            <div className="about-panel-list-item"><button onClick={() => setCurrentView('delivery')}><h4>Delivery Areas</h4><p>See suburbs and delivery fees</p></button></div>
            <div className="about-panel-list-item"><Link to="/terms"><h4>Terms & Conditions</h4><p>View our terms of service</p></Link></div>
        </>
    );
}

// ADD THIS ENTIRE BLOCK AFTER YOUR AboutPanel COMPONENT

// REPLACE your old TermsPage function with this new, complete version
...
function TermsPage() {
    // Reusable styles for this page to keep the code clean
    ...
    // Data for the Table of Contents for easy generation
    const tocItems = [
    ...
    // Helper function to create consistent IDs for linking
...
// --- NAVBAR COMPONENT ---
function Navbar({ onAboutClick, onMenuClick, onLoginClick }) {
    const { cart } = useCart();
        ...
        <nav style={{ backgroundColor: 'var(--background-dark)', padding: '0.5rem 1.5rem', borderBottom: `1px solid var(--brand-pink)`, position: 'sticky', top: 0, zIndex: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                    <ThemeSwitcher />
                    <Link to="/" onClick={onMenuClick} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center' }}><img src="/pizza-peppers-logo.jpg" alt="Pizza Peppers Logo" style={{ height: '3.5rem' }} /></Link>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                    ...
                    {currentUser ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            <span style={{ color: 'var(--text-medium)', fontSize: '0.9rem' }}>Hi, {currentUser.displayName.split(' ')[0]}</span>
                            <button onClick={logout} style={{all: 'unset', color: 'var(--brand-pink)', cursor: 'pointer', fontWeight: '500'}}>Logout</button>
                        </div>
                    ) : (
                        <button onClick={onLoginClick} style={{all: 'unset', color: 'var(--brand-neon-green)', cursor: 'pointer', fontWeight: '700'}}>Login</button>
                    )}
                </div>
            </div>
        </nav>
...
// --- NEW DATA FETCHING LOGIC (USING PROXY) ---
    useEffect(() => {
        const fetchMenu = async () => {
            try {
                // This now fetches from the local proxy path '/api/menu'.
                // Vite's proxy will handle forwarding this to the live server
                // and securely adding your secret API key in the background.
                // Use same-origin /public in dev (proxied by Vite), full URL in prod
    ...
    // Handlers to switch the right panel view
    const showOrderPanel = () => {
        setSelectedItem(null); // Deselect any item to ensure order panel shows
        ...
        <>
            ...
            <div className="app-grid-layout">
                <div className="left-pane">
                    <Navbar onAboutClick={showAboutPanel} onMenuClick={showOrderPanel} onLoginClick={() => setIsLoginModalOpen(true)} />
                    <main className="main-content-area">
                        {isLoading ? ( <p style={{ textAlign: 'center', fontSize: '1.2rem', marginTop: '4rem' }}>Loading menu...</p> ) : (
                            <Routes>
                ...
                <div className="right-sidebar">
                    <div className="order-panel-container">
                        {selectedItem ? (
                            ...
                            <AboutPanel onOpenHours={() => setIsHoursModalOpen(true)} onOpenDelivery={() => setIsDeliveryModalOpen(true)} isMapsLoaded={isMapsLoaded} />
                        ) : (
...
function App() {
  ...
  return (
    <Router>
      <ThemeProvider>
        <AuthProvider> {/* <-- THIS WRAPPER IS NEW */}
            <CartProvider>
                <AppStyles /> 
                <AppLayout isMapsLoaded={isMapsLoaded} />
            </CartProvider>
        </AuthProvider> {/* <-- AND ITS CLOSING TAG */}
      </ThemeProvider>
    </Router>
  );
}
